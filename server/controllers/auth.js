...
export const register = async (req, res) => {
  try {
    ...
    if (!firstName || !lastName || !email || !password || !imgUrl) {
      return res
        .status(400)
        .json({ success: false, message: "All fields are required" });
    }

    // Validate first name and last name (should be at least 2 characters long)
...
export const adminlogin = async (req, res) => {
  try {
    ...
    if (!admin) {
      return res.status(400).json({
        success: false,
        message: "Admin Not Found! Please Contact to Super Admin to Register",
      });
    }
    ...
    return res
      .status(200)
      .json({ success: true, message: "Login Successfull!", admin, token });
  } catch (err) {
    console.log(err);
    res.status(500).json({
      success: false,
  ...
  try {
    ...
    if (existEmail) {
      ...
      const mailOptions = {
        ...
        to: email,
        subject: "CodeNestX User Verification",
        html: `
    <html>
      <head>
        <style>
      ...
      <body>
        <div class="container">
          <h2 style="color: #004aad;">CodeNestX User Verification</h2>
        ...
        <div class="footer">
          ...
          <p>
            ...
            <a href="#">Get the CodeNestX app</a>
          </p>
          <p>CodeNestX Corporation © 2024</p>
        </div>
      </body>
    </html>
  `,
